...
use Swoole;
use NoahBuscher\Macaw\MacawSwoole;
class SwooleServer
{
    ...
    public function __construct($host = '0.0.0.0', $port = 9504)
    {
        $this->_setting = array(
            'host' => $host,
            'port' => $port,
    ...
    {
        error_log(date("Y-m-d H:i:s"), 3, '/tmp/swoole_test.log');
        echo "[#{$workerId}]. running. on ";
        ...
        // echo SWOOLE_PATH . DIRECTORY_SEPARATOR . 'ThinkPHP' . DIRECTORY_SEPARATOR . 'ThinkPHP.php';
    ...
    */
    ...
    {
        $data = '<h1>Hello Swoole.</h1>';
        $this->response($serv, $client_id, $data);//封装并发送HTTP响应报文
    }
    ...
    function onRequest(\swoole_http_request $request, \swoole_http_response $response)
    {
        ...
        MacawSwoole::get('/fuck', function() {
            echo '99999999999999';
            $content = 'success';

        });
        MacawSwoole::dispatch();
        ...
        /*if (isset($map[$path])) {
            ...
            include  'src/pages/'.$map[$path].'.php';
        ...
        $response->end("<h1>Hello Swoole.$content </h1>");
    ...
    public function prepareRequest($request)
    {
        ...
        return (new Web)->makeRequest($config);
    }
